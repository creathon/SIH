...
public class TeamMemberController {
	...
	public ModelAndView saveTeamMember(@ModelAttribute("teamMember") TeamMember teamMember, ModelMap model,
		...
		if(sessionBean!=null){
			...
			/*if(teamMemberList==null || teamMemberList.size()==0){
				mav.addObject("error", "Sorry!! you don't have any crew members");
			}*/
	...
	public ModelAndView teamMemberList(ModelMap model,HttpServletRequest request) {
		...
		if(sessionBean!=null){
			List<TeamMember> teamMemberList = teamMemberService.listOfTeamMemberByProdId(sessionBean.getProductionHouse().getId());
			if(teamMemberList==null || teamMemberList.size()==0){
				mav.addObject("error", "Sorry!! You have not added any crew members yet.");
			}
	...
	@RequestMapping(value = "ajax/deleteTeamMember")
	@ResponseBody
	public Integer deleteTeamMember(@RequestParam("id") Integer teamMemberId)
	{
		System.out.println("inside ajax call for delete team member");
